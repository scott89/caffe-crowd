...
int feature_extraction_pipeline(int argc, char** argv) {
  ...
  if (argc < num_required_args) {
    ...
    "Usage: extract_features_with_keys  pretrained_net_param"
    "  feature_extraction_proto_file  extract_feature_blob_name1[,name2,...]"
    "  save_feature_dataset_name1[,name2,...]  num_mini_batches key_file db_type"
    "  [CPU/GPU] [DEVICE_ID=0]\n"
  ...
  std::ifstream key_file(argv[++arg_pos]);
  std::string a_key;
  std::vector<std::string> keys;
  while (std::getline(key_file, a_key)) {
    keys.push_back(a_key);
  }
  LOG(INFO) << keys.size() << " keys in total.";
  ...
  for (int batch_index = 0; batch_index < num_mini_batches; ++batch_index) {
    ...
    for (int i = 0; i < num_features; ++i) {
      ...
      for (int n = 0; n < batch_size; ++n) {
        ...
        try {
          txns.at(i)->Put(keys.at(image_indices[i]), out);
